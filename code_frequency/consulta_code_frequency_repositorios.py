...
def consulta_code_frequency_repositorios(arquivo):
    arquivo_saida = []
    erro = False
    lista_code_frequency = []
    existe_code_frequency = False
    ...
    for x in (range(len(arquivo))):
        ...
        url = arquivo[x]['url']
        urlprincipal = f'{str(url)}/stats/code_frequency' 
        ...
        headers = {
            'Authorization' : 'token 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',
        ...
        if type(dados_api) is int: # Caso ocorra algum erro, finaliza busca
            # 404 - Quando a página não é encontrada grava sem code_frequency
            # 451 - Página bloqueada por motivos legais
            if dados_api == 404 or dados_api == 451 or dados_api == 202:
                print("Erro: " + str(dados_api) + " - não impeditivo.")
                registro = {}
            ...
            items = dados_api
            print(f'Quantidade de items: {str(len(items))}')
...
arquivo_json_saida = consulta_code_frequency_repositorios(arquivo_json)

nome_arquivo_saida = f'code_frequency-{str(nome_arquivo)}'
