...
def consulta_tags_repositorios(arquivo):
    arquivo_saida = []
    erro = False
    lista_tags = []
    existe_tags = False
    ...
    for x in (range(len(arquivo))):
        ...
        for i in range(1,11):
            ...
            url = arquivo[x]['url']
            urlprincipal = f'{str(url)}/tags?page={str(i)}&per_page=100' 
            ...
            if type(dados_api) is int: # Caso ocorra algum erro, finaliza busca
                # 404 - Quando a página não é encontrada grava sem tags
                ...
                if len(items) == 0: #Verifica se lista está vazia e finaliza busca
                    ...
                    if i == 1: # Grava o registro sem tags
                        registro = {}
                        registro['id']           = arquivo[x]['id'] 
                        registro['name']         = arquivo[x]['name']
                        registro['url']          = arquivo[x]['url']
                        ...
                        if existe_tags: # Gera registro do repositorio com suas tags
                            registro = {}
                            registro['id']           = arquivo[x]['id'] 
                            registro['name']         = arquivo[x]['name']
                            registro['url']          = arquivo[x]['url']
                            registro['created_at']   = arquivo[x]['created_at']
                            registro['num_watchers'] = arquivo[x]['num_watchers']       
                            registro['tags']         = lista_tags
                            print(f'Quantidade de tags: {len(lista_tags)}')
                            arquivo_saida.append(registro)     
                    ...
                    existe_tags = True
                    # Pega todas as tags e insere na lista com o seu repositorio
                    for y in range(len(items)):       
                        lista_tags.append(items[y])
                    
                    if i == 10: # Se for a última pesquisa -
                       # Gera registro do repositorio com suas tags
                        registro = {}
                        registro['id']           = arquivo[x]['id'] 
                        registro['name']         = arquivo[x]['name']
                        registro['url']          = arquivo[x]['url']
...
arquivo_json_saida = consulta_tags_repositorios(arquivo_json)

nome_arquivo_saida = f'tags-{str(nome_arquivo)}'
