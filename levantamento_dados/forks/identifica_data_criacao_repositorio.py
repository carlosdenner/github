...
def gerar_data_criacao_repositorio(arquivo, arquivo_2):
    ...
    for i in range(len(arquivo_2)):
        ...
        if arquivo_2[i]['forked_from'] != repo_id_ant:
            print(arquivo_2[i]['forked_from'] )
            repo_id_ant =  arquivo_2[i]['forked_from'] 

            registro = list(filter(lambda x:x["id"] == arquivo_2[i]['forked_from'],arquivo))
