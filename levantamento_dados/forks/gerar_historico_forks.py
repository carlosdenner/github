...
def gerar_historico_forks(arquivo_json):
    arquivo_saida = []
    repo_id_ant = 0
    qtd_forks_ant = 0
    repo_ant = {}
    ...
    for i in range(len(arquivo_json)):
        ...
        if arquivo_json[i]['forked_from'] != repo_id_ant:
            ...
            if repo_id_ant != 0:
                ...
                for x in range(qtd_dias):
                    ...
                    registro = {}
                    registro['id']       = repo_ant['forked_from']
                    registro['data']     = data_string
                    registro['forks']    = qtd_forks_ant
                    arquivo_saida.append(registro)


            repo_id_ant = arquivo_json[i]['forked_from'] 
            qtd_forks_ant = 0
            ...
            if arquivo_json[i]['data_criacao'] != arquivo_json[i]['data']:
                ...
                data_forks = datetime.datetime.strptime(arquivo_json[i]['data'],"%d-%m-%Y")
                data_criacao = datetime.datetime.strptime(arquivo_json[i]['data_criacao'],"%d-%m-%Y")

                if data_criacao < data_forks:
                    data = data_criacao
                    for x in range(qtd_dias+1):
                        data_string = datetime.datetime.strftime(data,"%d-%m-%Y")
                        registro = {}
                        registro['id']       = arquivo_json[i]['forked_from']
                        registro['data']     = data_string
                        if arquivo_json[i]['data'] == data_string:
                            registro['forks'] = int(arquivo_json[i]['quantidade_forks'])
                            qtd_forks_ant     = int(arquivo_json[i]['quantidade_forks'])
                        else:
                            registro['forks'] = 0
                        arquivo_saida.append(registro)
                        data = data + timedelta(days=1)
            ...
            for y in range(qtd_dias):
                ...
                registro['data']     = data_string
                if arquivo_json[i]['data'] == data_string:
                    qtd_forks_ant = qtd_forks_ant + int(arquivo_json[i]['quantidade_forks']) 
                registro['forks'] = qtd_forks_ant
                arquivo_saida.append(registro)     
...
print("Informe o nome do arquivo.json dos forks: ")
nome_arquivo_forks = input()

arquivo_json = ler_arquivo_json_tipo_1(nome_arquivo_forks)

arquivos_json_saida = gerar_historico_forks(arquivo_json)

nome_arquivo_forks_saida = f'saida-{str(nome_arquivo_forks)}'

gravar_arquivo_json(nome_arquivo_forks_saida,arquivos_json_saida)
